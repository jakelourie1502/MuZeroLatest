...
from global_settings import env_size, state_channels, proj_l1, proj_out, pred_hid,RND_output_vector
...
class JakeZero(torch.nn.Module):
    ...
    def __init__(self, representation, dynamic, prediction):
        ...
        self.RDN_prediction = nn.Sequential(nn.Conv2d(state_channels,state_channels,1,1,0),
                                ...
                                nn.Conv2d(state_channels,state_channels//4,3,1,1),
                                nn.ReLU(),
                                nn.BatchNorm2d(state_channels//4),
                                nn.Flatten(start_dim=1),
                                nn.Linear(env_size[0]*env_size[1]*state_channels//4, RND_output_vector,bias=False),
                                nn.Sigmoid())
