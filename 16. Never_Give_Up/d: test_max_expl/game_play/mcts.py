import time
import numpy as np
from global_settings import rdn_beta, value_only, manual_over_ride_play_limit, temperature_changes, play_limit_mcts, c1, c2 , gamma, actions_size, play_limit_mcts, exponent_node_n, use_policy, dirichlet_alpha, ucb_denom_k, reward_support, value_support
...
class MCTS:
    ...
    def expand(self,node):
        ...
        if node != self.root_node:
            ...
            rdn_value = max(0,(1-d) * rdn_beta * self.RDN_eval.evaluate(rdn_random,rdn_pred))
            node.exp_r = rdn_value
        ...
        # print('v', V)
        if node!= self.root_node:
            self.back_prop_rewards(node, V, node.exp_r)
        else:
    ...
    def back_prop_rewards(self, node, V, exp_r):
        ...
        node.exp_r = max(node.exp_r, exp_r)
        # print(v, node.r)
        node.Q_sum += v
        # print('q', node.Q_sum)
        node.N +=1
        node.Q = node.Q_sum / node.N + node.exp_r

        # print("mcts unit test: here are some samples of node V, r , Q: ", V, node.r,node.Q)
        if node != self.root_node:
            self.back_prop_rewards(node.parent, v, node.exp_r)
