...
class RDN_loss:
    ...
    def evaluate(self,random_output, predicted_output):
        return ((torch.mean((random_output-predicted_output)**2)**0.5 - self.mu) / self.siggy)

    def training(self, random_output, predicted_out, dones, weights):
        a = self.a
        mse = torch.mean(weights*(1-dones)*(random_output-predicted_out)**2)
